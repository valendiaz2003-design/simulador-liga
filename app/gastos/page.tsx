...
export default function GastosPage() {
  ...
  useEffect(() => {
    const applyBranding = () => {
      ...
      icon.rel = "icon";
      icon.type = "image/png";
      icon.href = "/images/financ-logo.png?v=financ3";
      document.head.appendChild(icon);

      const shortcut = document.createElement("link");
      shortcut.rel = "shortcut icon";
      shortcut.href = "/images/financ-logo.png?v=financ3";
      document.head.appendChild(shortcut);

      const apple = document.createElement("link");
      apple.rel = "apple-touch-icon";
      apple.href = "/images/financ-logo.png?v=financ3";
  ...
  async function cargarDatos() {
    ...
    if (cats && cats.length > 0) {
      ...
      setCategoriasGasto(
        unicos([
          ...categoriasGastoBase,
          ...nombres.filter((x) => !categoriasIngresoBase.includes(x)),
        ])
      );
  ...
  const movimientosMesAnterior = ordenados.filter((m) => m.fecha.startsWith(mesAnterior));
  ...
  const ingresosMesAnterior = sumar(movimientosMesAnterior, "ingreso");
  const gastosMesAnterior = sumar(movimientosMesAnterior, "gasto");
  ...
  const categoriaMayor = categoriasMes[0];
  const diasConGasto = diasMes.length;
  const promedioDiario = diasConGasto ? gastosMes / diasConGasto : 0;

  const variacionGastos =
    gastosMesAnterior > 0 ? ((gastosMes - gastosMesAnterior) / gastosMesAnterior) * 100 : 0;
  ...
  const movimientosFiltrados = useMemo(() => {
    return ordenados
      .filter((m) => {
        ...
        if (!fechaDesde && !fechaHasta && !m.fecha.startsWith(mes)) return false;
        if (filtroTipo !== "todos" && m.tipo !== filtroTipo) return false;
        if (filtroCategoria !== "todas" && m.categoria !== filtroCategoria) return false;

        const q = busqueda.toLowerCase().trim();
        if (q) {
          const texto = `${m.fecha} ${m.tipo} ${m.categoria} ${m.detalle} ${m.monto}`.toLowerCase();
  ...
  }, [ordenados, mes, fechaDesde, fechaHasta, filtroTipo, filtroCategoria, busqueda]);

  const categoriasFiltro = unicos([
    ...categoriasGasto,
    ...categoriasIngreso,
    ...movimientos.map((m) => m.categoria),
  ]);

  const resumenInteligente = crearResumenInteligente({
    gastosMes,
    gastosMesAnterior,
    ingresosMes,
    ingresosMesAnterior,
  ...
  async function guardar(e: React.FormEvent) {
    ...
    setMovimientos(
      editando ? movimientos.map((m) => (m.id === editando ? nuevo : m)) : [nuevo, ...movimientos]
    );
  ...
  function exportarCSV(data = movimientosFiltrados, nombre = `financ-${mes}.csv`) {
    const filas = [
      ["Fecha", "Tipo", "Categoria", "Detalle", "Monto"],
      ...data.map((m) => [m.fecha, m.tipo, m.categoria, m.detalle, String(m.monto)]),
    ];

    descargarArchivo(nombre, filas.map((f) => f.join(";")).join("\n"), "text/csv");
  }
  ...
  return (
    <main className="min-h-screen bg-[#f2f8fc] text-[#08244a]">
      <div className="mx-auto max-w-7xl px-4 py-5">
        ...
        {vista === "resumen" && (
          <section className="space-y-5">
            <Panel title="Resumen inteligente">
              <div className="grid gap-3 md:grid-cols-2">
                {resumenInteligente.map((txt) => (
                  <div key={txt} className="rounded-2xl bg-[#f4fbff] p-4 text-sm font-semibold text-[#355577]">
                    {txt}
                  </div>
                ))}
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <Mini title="Gasto promedio/día" value={dinero(promedioDiario)} />
              <Mini title="Días con gasto" value={String(diasConGasto)} />
              <Mini title="Mayor categoría" value={categoriaMayor ? categoriaMayor[0] : "-"} />
              <Mini title="Variación gastos" value={`${Math.round(variacionGastos)}%`} />
            </div>
            ...
            <Panel title="Exportar / Backup">
              <div className="grid gap-3 md:grid-cols-2">
                <button onClick={() => exportarCSV(movimientosMes, `financ-${mes}.csv`)} className="rounded-2xl bg-[#075db5] px-5 py-3 font-black text-white">
                  Exportar CSV del mes
                </button>
                <button onClick={exportarBackupJSON} className="rounded-2xl bg-[#08244a] px-5 py-3 font-black text-white">
        ...
        {vista === "movimientos" && (
          <Panel title="Movimientos">
            <div className="mb-4 grid gap-3 md:grid-cols-3 xl:grid-cols-6">
              <input className="input xl:col-span-2" placeholder="Buscar..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />

              <select className="input" value={filtroTipo} onChange={(e) => setFiltroTipo(e.target.value as FiltroTipo)}>
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              <button onClick={() => exportarCSV(movimientosFiltrados, "financ-filtrado.csv")} className="rounded-2xl bg-[#075db5] px-5 py-3 font-black text-white">
              ...
              <button
                ...
                className="rounded-2xl bg-[#e9f7ff] px-5 py-3 font-black text-[#07559d]"
              >
                Limpiar filtros
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              {!movimientosFiltrados.length && <Empty>No hay movimientos con esos filtros.</Empty>}
            </div>
          </Panel>
        )}
        ...
        {vista === "plan" && (
          <section className="space-y-5">
            <Panel title="Presupuesto mensual">
              ...
              <input className="input" type="number" step="0.01" placeholder="Ej: 300" value={presupuestoMes || ""} onChange={(e) => guardarPresupuesto(Number(e.target.value))} />
      ...
      <style jsx global>{`
        ...
        .control:focus {
          border-color: #008bd2;
          box-shadow: 0 0 0 4px rgba(0, 139, 210, 0.12);
        }
      `}</style>
    </main>
  );
...
function obtenerMesAnterior(mes: string) {
  const [y, m] = mes.split("-").map(Number);
...
function crearResumenInteligente(args: {
  gastosMes: number;
  ...
  if (args.presupuestoMes > 0) {
    if (args.restantePresupuesto >= 0) {
      out.push(`Vas dentro del presupuesto. Te quedan ${dinero(args.restantePresupuesto)} para el resto del mes.`);
    } else {
      out.push(`Te pasaste del presupuesto por ${dinero(Math.abs(args.restantePresupuesto))}.`);
    }
  } else {
    out.push("Todavía no configuraste presupuesto para este mes.");
  }

  if (args.disponibleHoy >= 0) {
    out.push(`Hoy todavía podés gastar ${dinero(args.disponibleHoy)} según tu ritmo de presupuesto.`);
  } else {
    out.push(`Hoy ya estás ${dinero(Math.abs(args.disponibleHoy))} por encima del ritmo diario.`);
  }

  if (args.categoriaMayor) {
    out.push(`Tu mayor gasto del mes es ${args.categoriaMayor[0]} con ${dinero(args.categoriaMayor[1])}.`);
  }

  if (args.gastosMesAnterior > 0) {
    const txt =
      args.variacionGastos >= 0
        ? `Gastaste ${Math.round(args.variacionGastos)}% más que el mes pasado.`
        : `Gastaste ${Math.abs(Math.round(args.variacionGastos))}% menos que el mes pasado.`;
    out.push(txt);
  }

  if (args.balanceMes >= 0) {
    out.push(`Tu balance mensual es positivo: ${dinero(args.balanceMes)}.`);
  } else {
    out.push(`Tu balance mensual es negativo: ${dinero(args.balanceMes)}.`);
  }
